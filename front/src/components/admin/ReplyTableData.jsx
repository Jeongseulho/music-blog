...
function ReplyTableData({ replyId, postId, content, userIp, registerDate, setAllReplyList }) {
  ...
  return (
    <tr className={`${darkMode ? 'dark' : ''} mb-0 table-row text-center`}>
      <td className=" border p-3 ">{replyId}</td>
      <td className=" w-[110px] border p-3 ">{postId}</td>
      <td className=" w-[320px] border p-3 ">{content}</td>
      <td className=" w-[190px] border p-3 ">{userIp}</td>
      <td className=" w-[130px] border p-3 ">{formatDateTime(registerDate)}</td>
